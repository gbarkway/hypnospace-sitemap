...
const makeDal = (path) => {
    ...
    return {
        ...
        getPages: (date, opts) => {
            ...
            if (opts.tags) {
                expressions.push(`tag.value IN (${'?,'.repeat(opts.tags.length).slice(0, -1)})`)
        ...
        getDates: () => {
            return promiseAll("SELECT DISTINCT date FROM page", []).then(rows => rows.map(row => row.date));
        },
